...
> **当前状态：v0.1 基础版本。** Hyprland 会话、安装器、备份与恢复、顶部菜单栏、应用启动器、通知中心、锁屏、壁纸及日常硬件集成已经可用。Dock 放大效果、拖动排序、统一控制中心、自动明暗主题和图形化安装器仍在路线图中，不会被描述为已完成功能。

## 功能概览

- 模块化 Hyprland 配置，输入、外观、窗口规则、快捷键和自启动相互独立
- 类 macOS 顶部菜单栏，包含工作区、网络、蓝牙、音量、电池、时钟和通知入口
- Walker 应用、文件、计算器和剪贴板搜索
- SwayNC 通知中心与媒体控件
...
- 如需安装 AUR 中的 Walker，请提前安装 `yay`；安装器不会静默下载或执行第三方 AUR 助手脚本
...
1. 注销当前桌面会话。
2. 在 SDDM 登录界面的会话菜单中选择 **Hyprland**。
3. 使用原有账号登录。
...
Walker 可能需要从 AUR 安装。安装器会先检查当前 Arch 官方仓库；只有软件包不在官方仓库时才调用已有的 `yay`，不会自动安装 AUR 助手。
