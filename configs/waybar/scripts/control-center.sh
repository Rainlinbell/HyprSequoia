...
choice=$(printf '%s\n' 'Wi-Fi and Network' 'Bluetooth' 'VPN' 'Brightness Up' 'Brightness Down' 'Sound Settings' 'Microphone' 'Night Shift' 'Toggle Light/Dark Mode' 'Notification Center' | walker --dmenu -p 'Control Center' 2>/dev/null || true)
case $choice in
  ...
  'Night Shift')
    if command -v gammastep >/dev/null 2>&1; then gammastep -O 4000 >/dev/null 2>&1 &
